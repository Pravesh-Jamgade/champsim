...
// // fill latency of mshr of Master LLC
// uncore.llc->LATENCY = LLC_LATENCY;

// fill and write latency of LLC Bank
for(int j=0; j< NUM_BANKS; j++)
    banks[j]->LATENCY = LLC_LATENCY * 0.9;// assuming
}
...
if (pr == page_table.end()) { // no VA => PA translation found 
    ...
    if (allocated_pages >= DRAM_PAGES) { // not enough memory
        ...
        for (uint32_t i=0; i<BLOCK_SIZE; i++) {
            ...
            for(int j=0; j< NUM_BANKS; j++)
                banks[j]->invalidate_entry(cl_addr);
        }
...
{
    banks.push_back(
        new CACHE("LLC_bank_"+to_string(i), LLC_SET, LLC_WAY, LLC_SET*LLC_WAY, LLC_WQ_SIZE, LLC_RQ_SIZE, LLC_PQ_SIZE, LLC_MSHR_SIZE)
    );
}
...
ooo_cpu[i].L1I.l1i_prefetcher_cache_fill = cpu_l1i_prefetcher_cache_fill;
    ...
    for(int j=0; j< NUM_BANKS; j++)
    {
        banks[j]->cache_type = IS_LLC;
        banks[j]->fill_level = FILL_LLC;
        banks[j]->MAX_READ = NUM_CPUS;
        banks[j]->upper_level_icache[i] = &ooo_cpu[i].L2C;
        banks[j]->upper_level_dcache[i] = &ooo_cpu[i].L2C;
        banks[j]->lower_level = &uncore.DRAM;

        banks[j]->llc_initialize_replacement();
        banks[j]->llc_prefetcher_initialize();
    }

    // OFF-CHIP DRAM
    uncore.DRAM.fill_level = FILL_DRAM;
    uncore.DRAM.upper_level_icache[i] = llc;
    uncore.DRAM.upper_level_dcache[i] = llc;
    for (uint32_t i=0; i<DRAM_CHANNELS; i++) {
