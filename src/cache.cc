...
{
  // MSHR is an object of PACKET_QUEUE
  // var(next_fill_index,SIZE,) is decalred in PACKET_QUEUE  variable MSHR_SIZE is declared in cache.h file
  // cpu = NUM_CPUS; in block.h
  // PACKET *entry, address is the variable of PACKET class.
  // handle fill
  uint32_t fill_cpu = (MSHR.next_fill_index == MSHR_SIZE) ? NUM_CPUS : MSHR.entry[MSHR.next_fill_index].cpu;
  if (fill_cpu == NUM_CPUS)
    return; // MSHR is full or not.
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    { // IS_LLC and LLC_WAY are macros defined in cache.h,
...
#ifdef LLC_BYPASS
    if ((cache_type == IS_LLC) && (way == LLC_WAY))   //if (way==LLC_WAY) means set is full, because if set is full then we will apply "bypass".
    ...
    // is this dirty?           victim is dirty or not.
    if (block[set][way].dirty)
    {
      ...
      if (lower_level)
      {
        ...
        else
        {
          ...
          writeback_packet.address = block[set][way].address;   //victim address placed into write queue  address(Tag+set).
          writeback_packet.full_addr = block[set][way].full_addr;  //full address (Tag+Set+BlockOffset).
          writeback_packet.data = block[set][way].data;
          writeback_packet.instr_id = MSHR.entry[mshr_index].instr_id;
          writeback_packet.ip = 0; // writeback does not have ip
          writeback_packet.type = WRITEBACK;   //WRITEBACK in memory_class.h
...
//---------------------------------------------------------------Dikshit (End of Handel_writeback)--------------------------------------//
...
void CACHE::handle_read()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if (read_cpu == NUM_CPUS)   //if a requets is shared between more than one cpu...if (read_cpu==NUM_CPUS) means that request is no more required.(DK); 
      return;
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      if (way >= 0)   //if not hit then check_hit  will return -1, that's why we use (way>=0) for recognizing the hit.
      { // read hit

        if (cache_type == IS_ITLB)       //finding the hit for whichever cache type having this.
        {
        ...
        else if (cache_type == IS_STLB)   //STLB---Second TLB.
        ...
        if (RQ.entry[index].type == LOAD)   //on being hit at any level we are generating the prefetch request.
        {
          if (cache_type == IS_L1I)
        ...
        // update replacement policy  //on cache hit promotion will be held by cache controller.(DK) 
        if (cache_type == IS_LLC)
        {
        ...
        if (block[set][way].prefetch)
        {
          ...
          {
            perSetAccuracy[set][persetpfusefull]++; //DK
            pfCount++;//DK
          }
          //DK
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
    {
      ...
      { // prefetch hit
        ...
        // // update replacement policy //Changes sugggested by T.V Sir
        // if (cache_type == IS_LLC)
        // {
...
//--------------------------------------------------------------------Dikshit(End of Handle_prefetch)------------------------------//
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
  ...
  if (block[set][way].prefetch && (block[set][way].used == 0)){
    ...
    {
      perSetAccuracy[set][persetpfuseless]++;
    }
    //-------DK------------------//
  }
...
int CACHE::check_hit(PACKET *packet)
{
  ...
  int match_way = -1;//NUM_SET contain the number for sets for particular cache (ex: - for LLC NUM_SET have value 2048, for L2 cache 1024)
  if (NUM_SET < set)//that's why we are checking it.
  {
    cerr << "[" << NAME << "_ERROR] " << __func__ << " invalid set index: " << set << " NUM_SET: " << NUM_SET;
    //cerr is the standard error stream which is used to output the errors. It is an instance of the ostream class.
...
{
  pf_requested++;
  //uint64_t set=get_set(pf_addr);//DK
  ...
  if (PQ.occupancy < PQ.SIZE)
  {
    if ((base_addr >> LOG2_PAGE_SIZE) == (pf_addr >> LOG2_PAGE_SIZE))
    {
      ...
      pf_issued++;
      //perSetAccuracy[set][persetTotalcount]++; //DK
...
void CACHE::add_mshr(PACKET *packet)
{
  ...
  //MSHR_OCC[MSHR.occupancy]++;//DK
  //cout<<"MSHR_OCCUPANCY for: "<<MSHR.occupancy<<" "<<MSHR_OCC[MSHR.occupancy]<<endl;
  //cout<<"MSHR_OCCUPANCY for: "<<MSHR.occupancy<<" "<<MSHR_OCC[MSHR.occupancy]<<endl; //DK
  /*cache_type==IS_LLC&&*/
  /*if(warmup_complete[packet->cpu])
  {
    if(MSHR.occupancy<LLC_MSHR_SIZE)
        MSHR_OCC[MSHR.occupancy]++;//DK
    else {
        Out_Of_MSHR++;
    }  
...
void CACHE::printMshr_state()
{
  if(cache_type==IS_LLC)
        {
            cout<<"MSHR_Occupancy(LLC):- "<<endl<<endl;//(LLC)
            float sum=summ(MSHR_OCC,LLC_MSHR_SIZE);
            cout <<left <<setw(20) << "Bucket Number" <<setw(20) << "Occupancy"<<setw(20)<<"(%)Occupancy" <<endl;
    cout <<setw(60) <<setfill('-') << "" <<setfill(' ') <<endl;
            for(int i=0;i<LLC_MSHR_SIZE;i++)
            {
                cout << left <<setw(20) <<i << setw(20) << MSHR_OCC[i] <<setw(20)<<fixed <<setprecision(2)<<(MSHR_OCC[i]/sum)*100<<endl;
            }
            cout<<"Total Occupancy value: "<<sum+Out_Of_MSHR<<endl;
            cout<<"out of MSHR Size: "<<Out_Of_MSHR<<endl;
            cout<<endl;
        }

        if(cache_type==IS_L2C)
        {
            cout<<"MSHR_Occupancy(L2C): "<<endl<<endl;//(L2C)
            float sum=summ(MSHR_OCC,L2C_MSHR_SIZE);
            cout <<left <<setw(20) << "Bucket Number" <<setw(20) << "Occupancy"<<setw(20)<<"(%)Occupancy" <<endl;
    cout <<setw(60) <<setfill('-') << "" <<setfill(' ') <<endl;
        ...
        if(cache_type==IS_L1D)
        {
            cout<<"MSHR_Occupancy(L1D):- "<<endl<<endl;//(L1D)
            float sum=summ(MSHR_OCC,L1D_MSHR_SIZE);
            cout <<left <<setw(20) << "Bucket Number" <<setw(20) << "Occupancy"<<setw(20)<<"(%)Occupancy" <<endl;
    cout <<setw(60) <<setfill('-') << "" <<setfill(' ') <<endl;
            for(int i=0;i<L1D_MSHR_SIZE;i++)
            {
                cout << left <<setw(20) <<i << setw(20) << MSHR_OCC[i] <<setw(20)<<fixed <<setprecision(2)<<(MSHR_OCC[i]/sum)*100<<endl;
            }
            cout<<"out of MSHR Size: "<<Out_Of_MSHR<<endl;
        ...
        if(cache_type==IS_L1I)
        {
            cout<<"MSHR_Occupancy(L1I):- "<<endl<<endl;//(L1I)
            float sum=summ(MSHR_OCC,L1I_MSHR_SIZE);
            cout <<left <<setw(20) << "Bucket Number" <<setw(20) << "Occupancy"<<setw(20)<<"(%)Occupancy" <<endl;
    cout <<setw(60) <<setfill('-') << "" <<setfill(' ') <<endl;
            for(int i=0;i<L1I_MSHR_SIZE;i++)
            {
                cout << left <<setw(20) <<i << setw(20) << MSHR_OCC[i] <<setw(20)<<fixed <<setprecision(2)<<(MSHR_OCC[i]/sum)*100<<endl;
            }
            cout<<"out of MSHR Size: "<<Out_Of_MSHR<<endl;
