    ...
    */
    void print(IntPtr cycle = 0, string tag="end"){
        ...
        for(auto e: pc_to_write){
            ...
            cost_pc.push_back({e.second, e.first});
        }
        avg_wr_per_pc /= (double)pc_to_write.size();

        std::sort(cost_pc.begin(), cost_pc.end(), [](pii a, pii b){
        ...
        if(tag == "end"){
            ...
            f<<"pc,write,dead-when-evicted,alive-when-evicted,total_pc,top10,top_pc\n";
