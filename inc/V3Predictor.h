    ...
    void epoc_end_judgement_day(IntPtr cycle){
        ...
        for(auto page: two_level_gate){
            for(auto pc: page.second){
                ...
                if(pred)
                    judgement[page.first][pc.first].set_var_prediction(PREDICTION::ALIVE);
                else 
                    judgement[page.first][pc.first].set_var_prediction(PREDICTION::DEAD);
            }
        }
        ...
        for(auto page: gate_to_life){
            for(auto pc: page.second){
                ...
                judgement[page.first][pc.first].set_var_dead_or_alive(pkt_life);
            }
        }
