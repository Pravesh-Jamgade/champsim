...
struct CacheBlock {
  ...
  std::optional<uint64_t> lookup(uint64_t offset_within_base_addr) const {
    auto it = pte.find(offset_within_base_addr);
    if (it == pte.end()) return std::nullopt;
    return it->second;
  }
...
struct Page {
  ...
  std::optional<uint64_t> lookup(uint64_t offset_within_base_addr) const {
    if (offset_within_base_addr >= PAGE_SIZE) return std::nullopt;
    const uint64_t cbid = block_index(offset_within_base_addr);
    auto it = blocks.find(cbid);
    if (it == blocks.end()) return std::nullopt;
    return it->second.lookup(offset_within_base_addr);
  }
  ...
  bool erase_offset(uint64_t offset_within_base_addr) {
    ...
    if (removed && it->second.empty()) blocks.erase(it);
    return removed;
  }
...
class PageTable {
 public:
  ...
  // Lookup physical page by (base page + offset)
  std::optional<uint64_t> lookup(uint64_t base_page_table_addr, uint64_t offset_within_base_addr) const {
    const uint64_t base_page = page_align(base_page_table_addr);
  ...
  bool erase(uint64_t base_page_table_addr, uint64_t offset_within_base_addr) {
    ...
    bool removed = it->second.erase_offset(offset_within_base_addr);
    if (removed && it->second.empty()) pages.erase(it);
    return removed;
  }
