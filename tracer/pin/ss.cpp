...
// // Dictionary to store basic block execution counts saurabh 18-36
// std::map<ADDRINT, UINT64> basic_block_counts;

// // Callback function to be called for each instruction
// VOID CountBasicBlock(ADDRINT bbAddress) {
//     if (basic_block_counts.find(bbAddress) != basic_block_counts.end()) {
//         basic_block_counts[bbAddress]++;
//     } else {
//         basic_block_counts[bbAddress] = 1;
//     }
// }

// // Callback function to be called when the application exits
// VOID PrintBasicBlockCounts() {
//     std::cout << "Basic Block Execution Counts:" << std::endl;
//     for (const auto& entry : basic_block_counts) {
//         std::cout << "Address: 0x" << std::hex << entry.first << ", Count: " << entry.second << std::dec << std::endl;
//     }
// }
...
void EndInstruction()
{
    ...
    if(instrCount > KnobSkipInstructions.Value())
    {
        ...
        {
            tracing_on = false;
            // std::cout <<std::dec<< "Total on count reach: "<< countA << "," << countB << "," << countC << ","  << countD << '\n';
...
{
    // // Get the address of the basic block saurabh 414-417
    // ADDRINT bbAddress = INS_Address(ins);
    // // Instrument the instruction to call CountBasicBlock
    // INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)CountBasicBlock, IARG_ADDRINT, bbAddress, IARG_END);
...
{
    // PrintBasicBlockCounts();   //saurabh 495
    ...
    std::cout <<std::dec<< "Total: after application end "<< countA << "," << countB << "," << countC << ","  << countD << '\n';
}
