...
VOID set_flip(unsigned long long a, unsigned long long b, unsigned long long  c)
{
    if( a==b && b==c)
    {
      // std::cout << std::hex << "[SimPoint] " << a << "," << b << "," << c << '\n';
      // cout << start_flag << "-->";
    ...
    else 
    {
      // void handleMagic(unsigned long long a, unsigned long long b, int hint)
      {
        ...
        found[c] = true;
        if(a!=b)
          std::cout << std::hex << "[FOUND] " << start[c] << "," << end[c]  << "," << c << "," << found[c] << '\n';
      }
    }
}
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++) 
    {
        ...
        {
          if(INS_MemoryOperandIsWritten(ins, memOp)) {
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)addMagic, IARG_MEMORYOP_EA, memOp, IARG_UINT32, 1, IARG_END);
          }
          else if(INS_MemoryOperandIsRead(ins, memOp)) {
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)addMagic, IARG_MEMORYOP_EA, memOp, IARG_UINT32, 0, IARG_END);
          }
        }
    }
...
VOID Routine(RTN rtn, VOID *v)
{
    ...
    if(rc->_name.find("main") == string::npos)
    {
        return;
    }
...
VOID Fini(INT32 code, VOID *v)
{
    ...
    std::cout << std::dec << "XXX load(A),"<< countA[0] << ",store(A)," << countA[1] << ",load(B)," << countB[0] << ",store(B)," << 
              countB[1] << ",load(C)," << countC[0] << ",store(C)," << countC[1] << ",total_loads," << total_loads 
              << ",total_stores," << total_stores << ",total_instr," << instrCount <<  '\n';  
