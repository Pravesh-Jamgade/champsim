            ...
            for (int sgn = +1; sgn >= -1; sgn -= 2) {
                ...
                if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                    ...
                    if (cache->PQ.occupancy + cache->MSHR.occupancy < cache->MSHR.SIZE - 1 &&
                        ...
                        int ok = cache->prefetch_line(0, base_addr, pf_address, FILL_LLC, 0);
...
void CACHE::prefetcher_initialize() {
    ...
    LLC_PREF::prefetchers = vector<LLC_PREF::Bingo>(
        1, LLC_PREF::Bingo(REGION_SIZE >> LOG2_BLOCK_SIZE, MIN_ADDR_WIDTH, MAX_ADDR_WIDTH, PC_WIDTH, FT_SIZE,
                      AT_SIZE, PHT_SIZE, PHT_WAYS, PF_STREAMER_SIZE, LLC_PREF::DEBUG_LEVEL));
}

uint32_t CACHE::prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in) {
    if (LLC_PREF::DEBUG_LEVEL >= 2) {
        cerr << "CACHE::llc_prefetcher_operate(addr=0x" << hex << addr << ", ip=0x" << ip << ", cache_hit=" << dec
             << (int)cache_hit << ", type=" << (int)type << ")" << dec << endl;
        cerr << "[CACHE::llc_prefetcher_operate] CACHE{core=" << this->cpu << ", NAME=" << this->NAME << "}" << dec
             << endl;
    }
    ...
    LLC_PREF::prefetchers[0].access(block_number, ip);

    /* issue prefetches */
    LLC_PREF::prefetchers[0].prefetch(this, block_number);

    if (LLC_PREF::DEBUG_LEVEL >= 3) {
        LLC_PREF::prefetchers[0].log();
        cerr << "=======================================" << dec << endl;
    }
    return metadata_in;
}
...
uint32_t CACHE::prefetcher_cache_fill(
    ...
    //     LLC_PREF::prefetchers[i].eviction(evicted_block_number);
LLC_PREF::prefetchers[0].eviction(evicted_block_number);
    return metadata_in;
}
